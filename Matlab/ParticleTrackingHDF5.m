...
FolderRead = '2017.01.31 - Au NR particle tracking\';
...
% *************************************************************************
GroupInfo{1} = h5info(FilePathRead);
while max(size(GroupInfo{1}.Groups)) > 0

    list_options = cell(size(GroupInfo{1}.Groups));
    for i = 1:1:max(size(GroupInfo{1}.Groups))
        list_options{i} = GroupInfo{1}.Groups(i).Name;
    end
    
    selected_group = 1;
%     selected_group = menu('Choose group:', GroupInfo.Groups.Name);    
%     selected_group = listdlg('PromptString', 'Choose Group:',...
%                          'SelectionMode', 'multiple',...
%                          'ListString', list_options,...
%                          'InitialValue', 1,...
%                          'ListSize', [200,300]);
                     
    NewGroupInfo = cell(size(selected_group));
    for i = 1:1:max(size(selected_group))
        NewGroupInfo{i} = h5info(FilePathRead, GroupInfo{1}.Groups(selected_group(i)).Name);
    end
    GroupInfo = NewGroupInfo;
    
end

%% reading the data from the summary file
% *************************************************************************
number_of_variables = max(size(GroupInfo{1}.Datasets));
name_of_variables = cell(size(GroupInfo{1}.Datasets));
for i = 1:1:number_of_variables
    name_of_variables{i} = GroupInfo{1}.Datasets(i).Name;
end

index_spectra = find(strcmp(name_of_variables, 'spectra'));
DatasetPath = [GroupInfo{1}.Name '/' GroupInfo{1}.Datasets(index_spectra).Name];
counts = double(h5read(GroupInfo{1}.Filename, DatasetPath)');
wavelengths = double(h5readatt(GroupInfo{1}.Filename, DatasetPath, 'wavelengths'));
background = double(h5readatt(GroupInfo{1}.Filename, DatasetPath, 'background'));
reference = double(h5readatt(GroupInfo{1}.Filename, DatasetPath, 'reference'));
% in the summary.h5 file:
% spectra is already referenced and background corrected 

number_of_spectra = size(counts,1);
wavelengths = repmat(wavelengths,1,number_of_spectra)';
background = repmat(background,1,number_of_spectra)';
reference = repmat(reference,1,number_of_spectra)';

DatasetNames = cell(number_of_spectra,1);
for i = 1:1:number_of_spectra
    DatasetNames{i} = num2str(i-1,'%03.0f');
end
...
title({[FolderRead '\' FileNameRead], 'Raw Data'}, 'interpreter', 'none')
...
%% find peaks
% *************************************************************************
if any(strcmp(analysis_options(selected_analysis), 'Find Peaks'))    
    peak_particles = [];
    peak_wavelengths = [];
    peak_intensities = [];
    fit_coefficients = cell(number_of_spectra,1);
    data_fit = zeros(size(data_plot));
    fit_order = 1;
    
    figure
    for i = 1:1:number_of_spectra
%     for i = 1:70:number_of_spectra
% 	for i = 1:1:5
%         fit_coefficients{i} = polyfit(wavelengths(i,:),data_plot(i,:),fit_order);
%         data_fit(i,:) = polyval(fit_coefficients{i},wavelengths(i,:));
%         peak_wavelengths(i) = max(data_fit(i,:));

        plot(wavelengths(i,:),data_plot(i,:)), hold all
%         plot(wavelengths(i,:),data_fit(i,:)), hold all
        [peak_values,peak_locations] = findpeaks(data_plot(i,:),...
            'MinPeakDistance', 50, ...
            'MinPeakProminence', 5,...
            'SortStr', 'descend');
        
        for j = peak_locations
            peak_particles(end+1) = i;
            peak_wavelengths(end+1) = wavelengths(i,j);
            peak_intensities(end+1) = data_plot(i,j);
        end                
    end
    plot(peak_wavelengths,peak_intensities, 'k.', 'MarkerSize', 18)
    histogram(peak_wavelengths,10)
...
%     title({FolderRead, title_analysis}, 'interpreter', 'none')
    title({[FolderRead '\' FileNameRead], title_analysis}, 'interpreter', 'none')
    xlabel('Wavelength (nm)')
    ylabel(ytext)
    xlim([300,1050])
    if any(strcmp(analysis_options(selected_analysis), 'Optical Density'))
