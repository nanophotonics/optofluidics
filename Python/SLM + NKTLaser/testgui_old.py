...
class ExampleApp(QtGui.QMainWindow, testui.Ui_MainWindow):
    ...
    def slmupdate(self):
        distribution=np.real((LG(self.l,self.g,self.size,slmpix)))
        distribution=displace(angled(rotator(distribution,self.rotang), 
                                     self.verang, self.horang, slmpix), 
                                     self.verpos, self.horpos, slmpix)*intaim
