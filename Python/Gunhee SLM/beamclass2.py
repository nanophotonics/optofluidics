...
@author: Andrei
"""
#difference between beamclass is the method 'focus'
...
#addition of calibration against radial abberation. Search for Zernike polynomials - 2rho**2-1 indicates zernike polynomials R^0_2
    def focus(self, distribution, fval):
        def cart2pol(x, y):
            rho = np.sqrt(x**2 + y**2)
            phi = np.arctan2(y, x)
            return(rho, phi)
        ...
        xm, ym = np.meshgrid(x, y)
        rho, phi = cart2pol(xm,ym)
        callibration = fval*np.pi*(2*np.square(2*rho/nx)-1)
        return distribution+callibration
