...
@author: Andrei
"""
#The functionality of focus and store was added
...
import testui2 # This file holds our MainWindow and all design related things
...
class ExampleApp(QtGui.QMainWindow, testui2.Ui_MainWindow):
    def __init__(self):
    ...
    # It sets up layout and widgets that are defined
        ...
        self.storb.clicked.connect(self.store)
        self.hpb()
        self.rotb()
        self.vpb()
        self.vab()
    ...
    def slmupdate(self):
        ...
        distribution=beamgen.focus(distribution,self.focus)
...
#store the parameters for given l and g into the txt file
    def store(self):
        F = open('LG'+str(self.l)+str(self.g)+'.txt','w')
        F.write('Horizontal Position, Horizontal Angle, Vertical Position, Vertical Angle, Size, Focus, Rotation\n')
        F.write(str(self.horpos)+'\n')
        F.write(str(self.horang)+'\n')
        F.write(str(self.verpos)+'\n')
        F.write(str(self.verang)+'\n')
        F.write(str(self.size)+'\n')
        F.write(str(self.focus)+'\n')
        F.write(str(self.rotang)+'\n')
        print('Parameters are stored')
        F.close()
